...
export function Header({ showHistory, showAccount, onToggleHistory, onToggleAccount, onCloseHistory, onCloseAccount, onSelectHistory }: Props) {
  ...
  const triggerHistory = () => {
    ...
    setTimeout(() =>setHistoryPressed(false), 520);
    onToggleHistory();
  };
  const triggerAccount = () => {
    setAccountPressed(true);
    setTimeout(() =>setAccountPressed(false), 520);
    onToggleAccount();
  };
    ...
    <header className="fixed top-0 inset-x-0 h-[56px] flex items-center justify-between px-3 lg:px-6 bg-transparent border-0 shadow-none z-40 pointer-events-none">
        ...
        <div className="relative pointer-events-auto flex flex-col items-end">
          <LiquidGlass
            ...
            refractiveIndex={1.9}
            surface="convexSquircle"
            scaleRatio={historyPressed || accountPressed || showHistory || showAccount ? 1.85 : 1.42}
            specularOpacity={0.55}
            blur={1.0}
            className={`pointer-events-auto will-change-transform ${historyPressed || accountPressed ? "animate-[iosLiquidPress_520ms_cubic-bezier(0.34,1.56,0.64,1)]" : ""}`}
          >
            <div className="flex items-center gap-2 px-2 py-1.5 rounded-full">
              <button id="btn-history"onClick={triggerHistory} className={`hidden sm:inline-flex items-center gap-1.5 text-[12px] font-sans font-medium tracking-[0.01em] text-[#1A1D24] bg-[#F0F2F5] rounded-full px-3.5 py-1.5 shadow-[3px_3px_8px_#C8D0E0,-3px_-3px_8px_#FFFFFF] hover:shadow-[5px_5px_10px_#C8D0E0] transition-all active:scale-95 ${historyPressed ? "scale-95" : ""}`}>Lịch sử</button>
              <button id="btn-account"onClick={triggerAccount} className={`w-8 h-8 rounded-full bg-[#F0F2F5] flex items-center justify-center text-[11px] font-sans font-semibold text-[#1A1D24] shadow-[4px_4px_10px_#C8D0E0,-4px_-4px_10px_#FFFFFF] hover:shadow-[5px_5px_12px_#C8D0E0] transition-all overflow-hidden active:scale-95 ${accountPressed ? "scale-95" : ""}`}>
          ...
          {showAccount && (
            <div ref={accountRef} className="absolute top-full right-0 mt-2 w-64 z-50 animate-[cardIn_180ms_cubic-bezier(0.16,1,0.3,1)]">
              <div className="p-3 rounded-[16px] bg-[#F0F2F5]/90 backdrop-blur-md shadow-[8px_8px_24px_#C8D0E0,-8px_-8px_24px_#FFFFFF] border border-white/60 overflow-hidden">
          ...
          {showHistory && (
            <div ref={historyRef} className="absolute top-full right-0 mt-2 w-[340px] max-w-[90vw] z-50 flex flex-col max-h-[420px] animate-[cardIn_180ms_cubic-bezier(0.16,1,0.3,1)]">
              <div className="p-3 flex flex-col rounded-[16px] bg-[#F0F2F5]/90 backdrop-blur-md shadow-[8px_8px_24px_#C8D0E0,-8px_-8px_24px_#FFFFFF] border border-white/60 overflow-hidden">
              ...
              <div className="history-scroll overflow-y-scroll space-y-2 pr-2 overscroll-contain custom-scrollbar"style={{ maxHeight: "220px" }}>
                {[
                  ...
                  { title: "16tr • Đồ họa • RTX 3050", preview: "2 đề xuất · 1 tuần trước", time: "1 tuần trước" },
                  { title: "22tr • Gaming • 144Hz", preview: "1 đề xuất · 2 tuần trước", time: "2 tuần trước" },
                ].map((h) => (
      ...
      <style>{`@keyframes iosLiquidPress{0%{transform:scale(1)}15%{transform:scale(0.96)}35%{transform:scale(1.04)}55%{transform:scale(0.985)}100%{transform:scale(1)}} @keyframes liquidBreathe{0%,100%{transform:scale(1)}50%{transform:scale(1.012)}}`}</style>
    </>
  );
}
