...
export function HeroSection({ input, isListening, onInputChange, onSend, onToggleListen, onQuickSend }: Props) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 lg:py-12">
      <div className="w-full max-w-[760px] flex flex-col items-center text-center">
        ...
        <div className="w-full mt-8 relative">
          <div className="group relative flex items-center gap-2 p-2 pl-3 rounded-[24px] bg-[#F0F2F5] shadow-[8px_8px_24px_#C8D0E0,-8px_-8px_24px_#FFFFFF] border border-white/60 transition-all overflow-hidden">
            ...
            <input value={input} onChange={(e) => onInputChange(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") onSend(); }} placeholder="Ví dụ: 18 triệu học CNTT, cần nhẹ, chơi Valorant 144Hz..." className="flex-1 bg-transparent outline-none text-[14px] placeholder:text-[#9AA0AE] py-2 text-[#1A1D24] relative z-10" />
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-6">{["15tr học CNTT", "Gaming 20tr RTX 4060", "Mỏng nhẹ <1.4kg OLED", "PC 25tr render Premiere"].map((s) => (
          <button key={s} onClick={() => onQuickSend(s)} className="text-[12.5px] px-3.5 py-2 rounded-full bg-[#F0F2F5] shadow-[3px_3px_8px_#C8D0E0,-3px_-3px_8px_#FFFFFF] text-[#6B6F7B] hover:shadow-[5px_5px_10px_#C8D0E0] transition-shadow">{s}</button>
        ))}</div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full mt-10 text-left">{[{ t: "Hiểu đúng ý bạn", d: "Bạn nói kiểu thường ngày là được, TechWise tự hiểu và lọc ra máy hợp với bạn nhất" }, { t: "So sánh dễ hiểu", d: "Thấy ngay mỗi máy được gì, mất gì và có dễ hối hận sau này không" }, { t: "Đáng tiền thực tế", d: "Gợi ý dựa trên trải nghiệm dùng thật và giá bán, không chỉ thông số trên giấy" }].map((f) => (<div key={f.t} className="rounded-[20px] bg-[#F0F2F5] shadow-[6px_6px_16px_#C8D0E0,-6px_-6px_16px_#FFFFFF] border border-white/60 p-4 relative overflow-hidden"><div className="text-[12px] font-semibold text-[#1A1D24] tracking-tight relative z-10">{f.t}</div><div className="text-[12px] leading-relaxed text-[#6B6F7B] mt-1 relative z-10">{f.d}</div></div>))}</div>
      </div>
    </div>
  );
}
